...
そこでエネルギー汎関数を一般化して
\begin{equation}
    \mathop{\mathscr{L}}(w)=\int_{\Om} L(Dw)\,dx 
\end{equation}
という形のものを考えてみる．ここで$L\colon \Rset^n\to\Rset$は滑らかかつ一様凸だとする．このときに$\mathop{\mathscr{L}}$の極小点$u$は滑らかか？というのがHilbertの第19問題である．

Hilbertの第19問題の解決に向けた道筋について簡単に(informalにではあるが)説明しておきたい．極小点$u$が満たすEuler--Lagrange方程式は，形式的に計算してもわかるように
\begin{equation}
    \div(DL(Du))=\sum_{i,j=1}^n D_{ij}L(Du) D_{ij}u=0
\end{equation}
となる\footnote{この方程式は発散型方程式で弱解を用いて理解するべきものであるが，ここでは説明の簡単のため$u$の2階微分まで存在すると仮定して議論している．}．この方程式は今までとは違って非線形問題である．しかし，$a^{ij}(x)=D_{ij}L(Du(x))$とおいてみると$\sum_{i,j}a^{ij}D_{ij}u=0$という形であるので部分的には線形方程式の理論が使えそうだと期待できる．
...
=======
\begin{prop}[from $L^2$ to $L^\infty$]
>>>>>>> 94022e91d25f8a2ba5f58cada25422e2be352237
...
\begin{proof}
    ...
    \begin{proof}[Proof of claim]
        ...
        \begin{equation}
            \abs{\qty{w_k\geq1/2}\cap B_1}\geq \abs{\qty{w_{k+1}\geq0}\cap B_1}\geq \int_{B_1}w_{k+1}^2\geq\delta^2 
        \end{equation}
...
\end{equation}
を滑らかな一様凸関数，つまり定数$0<\lambda\leq \Lambda$が存在して
\begin{equation}
...
\end{equation}
が成り立つものとする．このとき作用汎関数$\mathscr{L}$を
\begin{equation}
    \mathscr{L}\colon H^1(\Om)\to \Rset,\quad \mathscr{L}(w)\coloneqq \int_{\Om}L(Dw(x))\,dx 
\end{equation}
で定める．なお以下の補題より$\mathscr{L}(w)$は有限値を取りwell-definedである．
\begin{lem}[basic estimates]
