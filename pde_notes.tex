...
\begin{proof}\footnote{証明は\cite[Theorem 2.14]{fr}を参照したが，一部改変している．初出は\cite{wan06}だそうである．}
    ...
    \paragraph{\#6}最後に\eqref{eq:laplacian_schauder_5}の第3項を評価する．$j=1,2,\ldots,k$に対して$h_j\coloneq u_j-u_{j-1}$は$B_{2^{-j}d}$で調和である．\eqref{eq:harmonic_int_estimate}で$\kappa=3$とした場合より
    \begin{align}
        \abs{\frac{D^2h_j(z)-D^2h_j(0)}{\abs{z}}}&\leq [h_{j}]_{3;B_{2^{-k-3}d}}\\
