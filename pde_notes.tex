...
\begin{proof}
    ...
    \end{equation}
    が成り立つ．test functionとして$\varphi=\qty(u-\sup_{\pOm}u)^+\in H^1_0(\Om)$を取る(ほんとに取れる？\footnote{取れる．$l=\sup_{\pOm}u=\inf\qty{k\in \Rset\mid (u-k)^+\in H^1_0(\Om)}$とおき，この$\inf$がachieveされることを見ればよい．minimizing sequence $\qty{k_j}_j$を取る．つまり$k_j\searrow l$かつ$(u-k_j)^+\in H^1_0(\Om)$．$H^1_0(\Om)$の定義より，各$j$に対して$\qty{\varphi^{(j)}_i}_i\subset C^\infty_0(\Om)$があり$\varphi^{(j)}_i\to (u-k_j)^+\inn H^1(\Om)$となる．
    \begin{equation}
...
\begin{proof}
    ...
    \begin{equation}
        \int_{\Rset^n}D\varphi\cdot \widetilde{A}_kD\widetilde{u}_k\,dx=\int D\varphi(x)\cdot A_k(z_k+\rho_kx)\frac{Du_k(z_k+\rho_kx)-Du_k(z_k-\rho_kx)}{\rho_k^\alpha[u_k]_{1,\alpha;B_1}}\,dx\eqqcolon \mathrm{I}-\mathrm{I\!I}\label{eq:blowup_div_eqn}
    \end{equation}
    と分ける．ここで
    \begin{align}
        \mathrm{I}&\coloneqq \frac{1}{\rho_k^\alpha [u_k]_{1,\alpha;B_1}}\int D\varphi(x)\cdot A_k(z_k+\rho_kx)Du_k(z_k+\rho_kx)\,dx\\
    ...
    \end{align}
    \begin{equation}
        \mathrm{I\! I}\coloneqq \frac{1}{\rho_k^\alpha[u_k]_{1,\alpha;B_1}}\int D\varphi(x)\cdot A_k(z_k+\rho_kx)Du_k(z_k-\rho_kx)\,dx\eqqcolon \mathrm{I\! I}_1+\mathrm{I\! I}_2,
    \end{equation}
    と$\mathrm{I\! I}$を更に分け，
    \begin{align}
        \mathrm{I\! I}_1&\coloneqq \frac{1}{\rho_k^\alpha[u_k]_{1,\alpha;B_1}}\int D\varphi(x)\cdot \qty(A_k(z_k+\rho_kx)-A_k(z_k-\rho_kx))Du_k(z_k-\rho_kx)\,dx,\\
        \mathrm{I\!I}_2&\coloneqq \frac{1}{\rho_k^\alpha[u_k]_{1,\alpha;B_1}}\int D\varphi(x)\cdot A_k(z_k-\rho_kx)Du_k(z_k-\rho_kx)\,dx\\
        &=\frac{\rho_k^{1-\alpha}}{[u_k]_{1,\alpha;B_1}}\int \varphi(x)f_k(z_k-\rho_kx)\,dx 
    \end{align}
...
\begin{thm}
    ...
    \end{equation}
    ならば$u\in C^{k+1,\alpha}(\Om)$であり，任意の$\Om'\ssubset \Om$に対して
    \begin{equation}
...
\end{thm}
\begin{proof}
    系\ref{cor:schauder}の証明と同様に，アプリオリ評価からの帰結である．高階のアプリオリ評価は帰納法による．
\end{proof}
\section{De Giorgi--Nashの評価とHilbertの第19問題}
\subsection{De Giorgi--Nashの評価}
次の発散型線形作用素
\begin{equation}
...
Schauder評価は$-Lu=0$であるとき，$a^{ij}$がHölder連続性から解$u$のHölder正則性が帰結するというものだった．

一方で，本節では係数$a^{ij}$の連続性を仮定せずに(有界性は仮定する)，解$u$の(局所)有界性および局所Hölder連続性を示す．
