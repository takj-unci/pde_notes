...
\begin{itemize}
    ...
    \item $\Lm^n \colon \text{$\Rset^n$上のLebesgue測度}$,    
    \item $\Hm^s  \colon \text{$\Rset^n$上の$s$次元Hausdorff測度}$,
    \item $B_R(x_0)\coloneq \qty{x\in\Rset^n\mid \abs{x-x_0}<R}$,
    \item $\omega_n\coloneq \Lm^n(B_1(0))=\pi^{n/2}/\varGamma(n/2)$．このとき$\Hm^{n-1}(\partial B_1(0))=n\omega_n$,
...
\begin{dfn}[Hölder (セミ)ノルム]
    ...
    \begin{align}
        \abs{u}_{0;\Omega}=[u]_{0;\Om}&\coloneq \sup_{x\in\Om}\abs{u(x)},\\
        [u]_{0,\alpha;\Om}&\coloneq \sup_{x\neq y\in\Om}\frac{\abs{u(x)-u(y)}}{\abs{x-y}^\alpha} \quad (\alpha\in(0,1])
    \end{align}
    としてHölderセミノルムを定める．また
    \begin{equation}
        \abs{u}_{0,\alpha;\Om}\coloneq \abs{u}_{0;\Om}+[u]_{0,\alpha;\Om}
    \end{equation}
    とHölderノルムを定義する．Hölder空間
    \begin{equation}
        C^{0,\alpha}(\Ombar)\coloneq \qty{u\in C(\Ombar)\mid [u]_{0,\alpha;\Om}<\infty} 
    \end{equation}
    の元$u\in C^{0,\alpha}(\Ombar)$を$\alpha$-Hölder連続関数と呼ぶ．$1$-連続関数はLipschitz連続関数のことである．

    また，$u\in C^k(\Ombar)\ (k\in\Nset)$に対して
    \begin{align}
        [u]_{k,0;\Om}&=[u]_{k;\Om}\coloneq \sup_{\abs{\gamma}=k}[D^\gamma u]_{0;\Om}\\
        \abs{u}_{k,0;\Om}&=\abs{u}_{k;\Om}\coloneq \sum_{j=0}^k [u]_{j;\Om},\\
        [u]_{k,\alpha;\Om}&\coloneq \sup_{\abs{\gamma}=k}[D^\gamma u]_{0,\alpha;\Om},\\
        \abs{u}_{k,\alpha;\Om}&\coloneq \abs{u}_{k;\Om}+[u]_{k,\alpha;\Om}\quad (\alpha\in(0,1])
    \end{align}
    と定義し，
    \begin{equation}
        C^{k,\alpha}(\Ombar)\coloneq\qty{u\in C^k(\Ombar)\mid [u]_{k,\alpha;\Om}<\infty}
    \end{equation}
    と定義する．

    最後に，局所Hölder連続関数の空間を
    \begin{equation}
        C^{k,\alpha}(\Om)\coloneq \qty{u\colon \Om\to\Rset\mid \forall \Om'\ssubset \Om;\ u\in C^{k,\alpha}(\overline{\Om'})}
    \end{equation}
\end{dfn}
...
\begin{dfn}[無次元ノルム]
    ...
    \begin{align}
        [u]_{k,0;\Om}'=[u]_{k;\Om}'&\coloneq R^k[u]_{k;\Om},\\
        [u]_{k,\alpha;\Om}'&\coloneq R^{k+\alpha}[u]_{k,\alpha;\Om}\quad (\alpha\in(0,1]),\\
        \abs{u}_{k,0;\Om}'=\abs{u}_{k;\Om}'&\coloneq \sum_{j=0}^k [u]_{j;\Om}',\\
        \abs{u}_{k,\alpha;\Om}'&\coloneq \abs{u}_{k;\Om}'+[u]_{k,\alpha;\Om}'\quad (\alpha\in(0,1])
    \end{align}
\end{dfn}
...
\begin{proof}
    ...
    したがって，$u\coloneq v_0$とおくと$u\in C^k(\Ombar)$であり，$D^\gamma u=v_\gamma$である．よって
    \begin{equation}
    ...
    $u\in C^{k,\alpha}(\Ombar)$かつ$\abs{u-u_m}_{k,\alpha;\Om}\to0$を示そう．$\abs{\gamma}=k$に対して各点収束の意味で$D^\gamma u_m\to D^\gamma u$で，
    \begin{equation}
        \sup_{m}[D^\gamma u_m]_{0,\alpha;\Om}\leq \sup_{m}\abs{u}_{k,\alpha;\Om}<\infty 
    \end{equation}
    であるから，補題\ref{lem:holder_sn_lsc}より$[D^\gamma u]_{0,\alpha;\Om}<\infty$である．したがって$u\in C^{k,\alpha}(\Ombar)$である．
...
\begin{lem}\label{lem:lip_dom}
    ...
    \begin{enumerate}[(D1)]
        ...
        \begin{itemize}
            ...
            \item $x_0=x,\ x_N=y$．
            \item $l(p)\coloneq \sum_{i=0}^{N-1}\abs{x_{i+1}-x_i}\leq L\abs{x-y}$．
        \end{itemize}
...
\begin{eg}\label{non-lip_dom}
    \begin{align}
        \Om_1&\coloneq \qty{(x_1,x_2)\in\Rset^2\mid \abs{x_1}<1,\ \abs{x_2}<1,\ x_2<\abs{x_1}^{1/2}}, \\
        \Om_2&\coloneq \qty{(x_1,x_2)\in\Rset^2\mid 0<x_1<1,\ -2<x_2<\sin(1/x_1)}
    \end{align}
...
\begin{thm}[Hölderセミノルムの補間不等式]\label{thm:holder_interpolation}
    ...
    が成り立つ．ここで
    \begin{equation}
        \tau\coloneq \frac{j+\beta}{k+\alpha},\quad \lambda\coloneq \frac{\rho_0}{R}
    \end{equation}
...
\begin{proof}
    ...
    $\qty{u_m}_{m}\subset C^{0,\alpha}(\Ombar)$を有界列とし，$M\coloneq \sup_{m}\abs{u_m}_{0,\alpha;\Om}<\infty$とする．$m\in\Nset,\ x,y\in\Om$に対して
    \begin{equation}
    ...
    \begin{align}
        ...
        &\leq \abs{u-u_{m_l}}_{0;\Om}+C(2M)^{\frac{\beta}{\alpha}}\abs{u-u_{m_l}}_{0;\Om}^{1-\frac{\beta}{\alpha}}\to 0\quad (D\to 0)
    \end{align}
    となる．
    ...
    \begin{equation}
        \sup_{m}\abs{D^\gamma u_m}_{0,\alpha';\Om}<\infty,\quad \text{where }\alpha'\coloneq \begin{cases}
            1 & (\text{if }j\leq k-1)\\
            \alpha & (\text{if }j=k)
        \end{cases}
    \end{equation}
    ...
    よって
    \begin{equation}
        \abs{u_{m_{l_1}}-u_{m_{l_2}}}_{j,\beta;\Om}=\sum_{i=0}^{j}[u_{m_{l_1}}-u_{m_{l_2}}]_{i;\Om}+[u_{m_{l_1}}-u_{m_{l_2}}]_{j,\beta;\Om}\to 0\quad (D_1,l_2\to \infty)
    \end{equation}
...
\begin{proof}
    ...
    $r\in (0,R)$に対して
    \begin{equation}
        \varphi(r)\coloneq \frac{1}{n\omega_n r^{n-1}}\int_{\partial B_r(x_0)}u\,d\sigma(x)=\frac{1}{n\omega_n}\int_{\abs{y}=1}u(x_0+ry)\,d\sigma(y)
    \end{equation}
...
\end{thm}
\begin{proof}
    $M\coloneq \sup_\Om u,\ \Om'\coloneq \qty{x\in\Om\mid u(x)=M}$とおくと，$\Om'$は$\Om$の相対閉集合であり，仮定より$\Omega'\neq\emptyset$である．$\Om'\subset\Om$が開集合であることを示そう．
...
\begin{proof}
    ...
    \begin{equation}
        w(x)\coloneq (\abs{\Delta u}_{0;\Om}+\abs{u}_{0;\pOm})\qty(\frac{R^2-(x_1-x_1^{(0)})^2}{2}+1)\quad (x\in\Om)
    \end{equation}
...
\begin{equation}
    \overline{x}\coloneq \frac{R^2}{\abs{x}^2}x \in R^n\setminus\overline{B} 
\end{equation}
とおく．
...
\begin{proof}
    ...
    \begin{equation}
        B_i\coloneq B_{di/k}(x_0)\quad (i=1,\ldots,k)
    \end{equation}
...
\begin{proof}
    $M\coloneq \sup_\Om u$，
    \begin{equation}
        \Om'\coloneq \qty{x\in\Om\mid u(x)=M}
    \end{equation}
...
\begin{thm}\label{thm:perron_sol}
    ...
    \begin{equation}
        u(x)\coloneq \sup_{v\in \mathcal{S}_{\varphi}}v(x)\quad (x\in\Om)
    \end{equation}
...
\begin{proof}
    ...
    背理法で示す．ある$z\in B$で$v(z)<u(z)$だと仮定する．このとき$\overline{u}\in \mathcal{S}_{\varphi}$があり，$v(z)<\overline{u}(z)$となる．$w_{k}\coloneq\max\qty{\overline{u},V_{m_k}}\in \mathcal{S}_{\varphi}$とおき，$W_{k}\in \mathcal{S}_{\varphi}$を$w_{k}$の$B$におけるharmonic liftingとする．
    \begin{equation}
...
\end{dfn}
\begin{rmk}
    barrierとlocal barrierは，次の意味で実質同じ概念である：$w$を$\xi$におけるlocal barrierとし，$B\ssubset N$を開球で$\xi\in B,\ m\coloneq\inf_{N\setminus B}w>0$を満たすものとする．このとき
    \begin{equation}
        \overline{w}(x)\coloneq\begin{cases}
...
\end{lem}
\begin{proof}
    $\varepsilon>0$とせよ．$M\coloneq\sup_{\pOm}\abs{\varphi}$とおく．$w$を$\xi$におけるbarrierとする．
...
\begin{dfn}
    ...
    \begin{equation}
        \Delta^h_{i}u(x)\coloneq \frac{u(x+he_i)-u(x)}{h},\quad (x,x+he_i\in \Om,\ h\neq0)
    \end{equation}
    と定義する．
\end{dfn}
...
\begin{proof}
    ...
    \paragraph{\#2}$\Om'\ssubset \Om$とせよ．$x_1,x_2\in \overline{\Om'}$で$u(x_1)=\sup_{\Om'}u,\ u(x_2)=\inf_{\Om'}u$となるものを取る．$\overline{\Om'}$はpath-connectedより$\Gamma\colon [0,1]\to \overline{\Om'}$があり，$\Gamma(0)=x_1,\ \Gamma(1)=x_2$となる．$d'=\dist(\Gamma,\pOm),\ R=d'/2$とすると，有限個の$0\leq t_1\leq \cdots\leq t_N\leq 1$があり，$z_i\coloneq \Gamma(t_i)$とすると
    \begin{equation}
...
\begin{equation}
    \osc_{\Om}u\coloneq \sup_{\Om}u-\inf_{\Om}u 
\end{equation}
とおく．
...
\begin{proof}
    ...
    $B_d\ssubset \Om$である．$x\in \Om'$が，$\abs{x}<d$を満たすならば，$2^{-k-1}d\leq \abs{x}<2^{-k}d$を満たす$k\in \Nset_{0}$を取る．oscillation decayの定数$\theta=\theta_n\coloneq\theta_{n,B_1,B_{1/2}}$を取る．注意\ref{rmk:scale_invariance_of_const}より
    \begin{align}
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \right.
    \end{equation}
    の解とし，$w\coloneq u-v\in H^1(\Om)$とすると$-\Delta w=f\inn \Om,\ w=0\on \pOm$である．
...
\begin{proof}
    ...
    \begin{proof}[Proof of the claim.]
        oscillation decayの定数$\theta=\theta_n\coloneq\theta_{n,B_1,B_{1/2}}$を取ると，注意\ref{rmk:scale_dependence_of_const}より
        \begin{equation}
...
\begin{proof}\footnote{証明は\cite[Theorem 2.14]{fr}を参照したが，一部改変している．初出は\cite{wan06}だそうである．}
    ...
    \end{equation}
    $\widetilde{u}(x)\coloneq u(0)+x\cdot Du(0)x+\frac{1}{2}x\cdot D^2u(0)x$を$0$まわりの2次までのTaylor展開とする．$u\in C^2(\Om)$より$u(x)-\widetilde{u}(x)=o(\abs{x}^2)\ (x\to0)$である．また$u_k-\widetilde{u}$は調和なので，\eqref{eq:harmonic_int_estimate}などより
    \begin{align}
    ...
    \paragraph{\#6}最後に\eqref{eq:laplacian_schauder_5}の第3項を評価する．$k=1,2,\ldots,k_0$に対して$h_k\coloneq u_k-u_{k-1}$は$B_{2^{-k}d}$で調和である．\eqref{eq:harmonic_int_estimate}で$\kappa=3$とした場合より
    \begin{align}
...
\begin{proof}
    ...
    \begin{equation}
        u_\varepsilon\coloneq u\ast \eta_\varepsilon\inn \Om_\varepsilon=\qty{x\in \Om\mid\dist(x,\pOm)>\varepsilon}
    \end{equation}
    とおく．つまり
    \begin{equation}
    ...
    \begin{equation}
        -\Delta u_{\varepsilon}=f\ast \eta_{\varepsilon}\eqcolon f_{\varepsilon}\inn \Om_{\varepsilon}
    \end{equation}
...
\begin{rmk}[定数係数の場合]\label{rmk:const_coefficients}
    ...
    \begin{equation}
        z=A^{1/2}x\in\widetilde{\Om}\coloneq \qty{A^{1/2}x\mid x\in \Om }
    \end{equation}
...
\begin{proof}
    ...
    \begin{equation}
        \transpose{P}D^2u(x_0)P=\diam(\lambda_1,\ldots,\lambda_n)\eqcolon D_{x_0},
    \end{equation}
    となる．$D^2u(x_0)\leq O$より$\lambda_i\leq0$である．$A(x)\colon (a^{ij}(x))_{i,j}$と定義し，
    \begin{equation}
        A^{P}(x)\coloneq \transpose{P}A(x)P\eqcolon (a_P^{ij}(x))_{i,j}
    \end{equation}
    ...
    \begin{equation}
        u_{\varepsilon}(x)\coloneq u(x)+\varepsilon e^{x_1}
    \end{equation}
    とおく．
    \begin{equation}
...
\begin{thm}[Schauder評価]\label{thm:schauder_non_divergence}
    ...
    である．
\end{thm}
以下，この定理を証明するが，$\Om=B_1,\ \Om'=B_{1/2}$として示す(covering argumentよりこれで十分である)．
...
\begin{lem}\label{lem:blowup}
    ...
    が成り立つ．ここで
    \begin{equation}
        C=C_{\delta,n,\alpha,\lambda,\Lambda,K}.
    \end{equation}
\end{lem}
\begin{proof}
    ...
    各$k$に対して
    \begin{align}
        \widetilde{a}^{ij}_k (x)&\coloneq a^{ij}_k(x_k+\rho_kx),\\
        \widetilde{u}_k(x)&\coloneq \frac{u_k(x_k+\rho_kx)-p_k(x)}{\rho_k^{2+\alpha}[u_k]_{2,\alpha;B_{1}}},\\
        \widetilde{f}_k(x)&\coloneq \frac{f_k(x_k+\rho_kx)-f_k(x_k)}{\rho_k^{\alpha}[u_k]_{2,\alpha;B_{1}}}\quad\quad (x\in B_{1/(2\rho_k)})
    \end{align}
    とおく．ここで$p_k$は
    \begin{equation}
        p_k(z)\coloneq u_k(x_k)+\rho_k\sum_{i}D_iu_k(x_k)z_i+\frac{1}{2}\rho_k^2\sum_{i,j}D_{ij}u_k(x_k)z_iz_j 
    \end{equation}
    で与えられる．
    ...
    である．また
    \begin{equation}
        \xi_k\coloneq \frac{y_k-x_k}{\rho_k}\in \partial B_1 
    \end{equation}
    とおくと
    \begin{equation}
...
\begin{proof}[Proof of Theorem \ref{thm:schauder_non_divergence}]
    Hölderセミノルムの亜種を
    \begin{equation}
        [u]^\ast_{2,\alpha;B_1}\coloneq \sup_{B_\rho(x_0)\subset B_1}\rho^{2+\alpha}[u]_{2,\alpha;B_{\rho/2}(x_0)}
    \end{equation}
    で定める．
    ...
    \begin{equation}
        z_j\coloneq x+\frac{j}{8}(y-x)\quad (j=0,\ldots,7)
    \end{equation}
    とおく．
    \begin{align}
...
\begin{proof}
    ...
    \end{equation}
    が成り立つ．test functionとして$\varphi=\qty(u-\sup_{\pOm}u)^+\in H^1_0(\Om)$を取る(ほんとに取れる？)．楕円型条件より
    \begin{equation}
...
\begin{proof}
    ...
    \end{equation}
    \begin{equation}
        \lambda<A_k\coloneq =(a^{ij}_k)_{i,j},\quad \sum_{i,j}|a^{ij}_k|^2\leq \Lambda^2,\quad \sum_{i,j}|a^{ij}_{k}|_{0,\alpha;B_1}\leq K, 
    \end{equation}
    \begin{equation}
    ...
    となるものを選んでおく．
    \begin{equation}
        \rho_k\coloneq \frac{\abs{x_k-y_k}}{2},\quad z_k\coloneq \frac{x_k+y_k}{2}
    \end{equation}
    とおく．
    ...
    \begin{equation}
        \widetilde{u}_k(x)\coloneq\frac{u_k(z_k+\rho_kx)+u_k(z_k-\rho_kx)-2u_k(z_k)}{\rho_k^{1+\alpha}[u_k]_{1,\alpha;B_{1}}},
    \end{equation}
    \begin{equation}
        \widetilde{a}^{ij}_k(x)\coloneq a^{ij}_k(z_k+\rho_kx),\quad \widetilde{A}_k(x)\coloneq (\widetilde{a}^{ij}_k)_{i,j}\quad (x\in B_{1/(2\rho_k)}),
    \end{equation}
    \begin{equation}
        \xi_k\coloneq \frac{y_k-x_k}{2\rho_k}\partial B_{1}
    \end{equation}
    とおく．
    ...
    \begin{equation}
        \int_{\Rset^n}D\varphi\cdot \widetilde{A}_kD\widetilde{u}_k\,dx=\int D\varphi(x)\cdot A_k(z_k+\rho_kx)\frac{Du_k(z_k+\rho_kx)-Du_k(z_k-\rho_kx)}{\rho_k^\alpha[u_k]_{1,\alpha;B_1}}\,dx\eqcolon \mathrm{I}-\mathrm{I\!I}\label{eq:blowup_div_eqn}
    \end{equation}
    と分ける．ここで
    \begin{align}
        \mathrm{I}&\coloneq \frac{1}{\rho_k^\alpha [u_k]_{1,\alpha;B_1}}\int D\varphi(x)\cdot A_k(z_k+\rho_kx)Du_k(z_k+\rho_kx)\,dx\\
    ...
    \end{align}
    \begin{equation}
        \mathrm{I\! I}\coloneq \frac{1}{\rho_k^\alpha[u_k]_{1,\alpha;B_1}}\int D\varphi(x)\cdot A_k(z_k+\rho_kx)Du_k(z_k-\rho_kx)\,dx\eqcolon \mathrm{I\! I}_1+\mathrm{I\! I}_2,
    \end{equation}
    と$\mathrm{I\! I}$を更に分け，
    \begin{align}
        \mathrm{I\! I}_1&\coloneq \frac{1}{\rho_k^\alpha[u_k]_{1,\alpha;B_1}}\int D\varphi(x)\cdot \qty(A_k(z_k+\rho_kx)-A_k(z_k-\rho_kx))Du_k(z_k-\rho_kx)\,dx,\\
        \mathrm{I\!I}_2&\coloneq \frac{1}{\rho_k^\alpha[u_k]_{1,\alpha;B_1}}\int D\varphi(x)\cdot A_k(z_k-\rho_kx)Du_k(z_k-\rho_kx)\,dx\\
        &=\frac{\rho_k^{1-\alpha}}{[u_k]_{1,\alpha;B_1}}\int \varphi(x)f_k(z_k-\rho_kx)\,dx 
    \end{align}
    とおく($\mathrm{I\! I}_2$の計算は$\mathrm{I}$のそれと同様)．

    以下，$\mathrm{I},\ \mathrm{I\!I}_1,\ \mathrm{I\!I}_2$の各項が$k\to\infty$で0に収束することを示す(したがって\eqref{eq:blowup_div_eqn}の左辺は0に収束する)．

    まずHölderの不等式などより($q_k^{-1}+q_k'^{-1}=1$として)
    \begin{align}
        \abs{\mathrm{I}}&\leq \frac{\rho_k^{1-\alpha}}{[u_k]_{1,\alpha;B_1}}\abs{\int \varphi(x)f_k(z_k+\rho_kx)\,dx}\\
        ...
        &\leq \max\qty{\Lm^n(\supp\varphi),\Lm^n(\supp\varphi)^{1-(1-\alpha)/n}}\cdot\norm{\varphi}_{\infty}\frac{\norm{f}_{q_k}}{[u_k]_{1,\alpha;B_1}}\rho_k^{1-\alpha-n/q_k}\\
        &\leq C(\varphi)\frac{\rho_k^{1-\alpha-n/q_k}}{k}\to0\quad (k\to\infty)  
    \end{align}
    である．なお仮定より$1-\alpha-n/q_k\geq0$である．同様に$\mathrm{I\!I}_2\to0$がわかる．
    \begin{align}
        \abs{\mathrm{I\!I}_1}&\leq \frac{1}{\rho_k^{\alpha}[u_k]_{1,\alpha;B_1}}\sum_{i,j}\abs{\int (a^{ij}(z_k+\rho_kx)-a^{ij}(z_k-\rho_kx))D_i\varphi(x)D_ju_k(z_k-\rho_kx)\,dx}\\
...
\end{proof}
\section{Nash--De Giorgi--Moser評価とHilbertの第19問題}
\end{document}